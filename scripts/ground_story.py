...
USER_AGENT = "premium-sticktalk/4.5 (research-first-story-engine)"
BANNED_FILLER = (
...
STOP_WORDS = {
    ...
    "vì", "sao", "không", "thể", "ngay", "đầu", "tiên", "chủ", "đề", "làm",
}
REQUEST_WRAPPERS = (
    r"^(?:hãy\s+)?(?:giúp\s+(?:tôi|mình)\s+)?(?:tạo|làm|dựng|viết|kể)\s+(?:một\s+)?(?:video|phim|clip)\s+(?:về|chủ\s*đề|nội\s*dung)?\s*[:\-–—]*\s*",
    r"^(?:video|phim|clip)\s+(?:về|chủ\s*đề|nội\s*dung)\s*[:\-–—]*\s*",
    r"^(?:chủ\s*đề|nội\s*dung|đề\s*tài)\s*[:\-–—]+\s*",
)


def _get(params):
    q = urllib.parse.urlencode({**params, "format": "json", "origin": "*"})
    r = urllib.request.Request(f"{API}?{q}", headers={"User-Agent": USER_AGENT})
    with urllib.request.urlopen(r, timeout=25) as x:
        return json.load(x)


def _normalize(v):
    return " ".join(re.findall(r"[\wÀ-ỹĐđ]+", unicodedata.normalize("NFKC", str(v)).casefold()))


def _tokens(v):
    return [t for t in _normalize(v).split() if len(t) > 1]


def _strip_request_wrapper(topic):
    s = re.sub(r"\s+", " ", str(topic or "")).strip(" .,:;-_\"'“”")
    previous = None
    while s and s != previous:
        previous = s
        for pattern in REQUEST_WRAPPERS:
            s = re.sub(pattern, "", s, flags=re.I).strip(" .,:;-_\"'“”")
    return s


def subject_from_prompt(topic):
    s = _strip_request_wrapper(topic)
    changed = True
    while changed:
        changed = False
        low = s.casefold()
        for p in GENERIC_PREFIXES:
            ...
            if low.startswith(marker):
                s = s[len(marker):].strip(" .,:;-_\"'“”")
                changed = True
                break
    low = s.casefold()
    cuts = [low.find(m) for m in GENERIC_CONNECTORS if low.find(m) > 0]
    if cuts:
        s = s[:min(cuts)].strip(" .,:;-_\"'“”")
    if ":" in s:
...
def _candidate_score(title, subject, i):
    tn, sn = _normalize(title), _normalize(subject)
    st = [t for t in _tokens(subject) if t not in STOP_WORDS]
    tt = set(_tokens(title))
    overlap = sum(t in tt for t in st)
    score = overlap * 20
    if tn == sn:
        score += 100
    elif sn in tn:
        score += 45
    if st and overlap == len(st):
        score += 35
    return (-score, i)
...
def _validate_resolution(topic, subject, canonical):
    st = [t for t in _tokens(subject) if t not in STOP_WORDS]
    ct = set(_tokens(canonical))
    overlap = [t for t in st if t in ct]
    if not st:
        return
    minimum = 1 if len(st) <= 2 else max(2, (len(st) + 1) // 2)
    if len(overlap) < minimum:
        raise RuntimeError(
            f"Wikipedia đã trả về sai chủ đề. Yêu cầu='{topic}', chủ thể='{subject}', kết quả='{canonical}'."
        )


def research_topic(topic):
    subject = subject_from_prompt(topic)
    print(f"RESEARCH SUBJECT: {subject}")
    variants = [f'intitle:"{subject}"', f'"{subject}"', subject]
    combined = []
    seen = set()
    for query_text in variants:
        search = _get({"action": "query", "list": "search", "srsearch": query_text, "srlimit": 10, "utf8": 1})
    ...
    ranked = sorted(enumerate(combined), key=lambda p: _candidate_score(str(p[1].get("title") or ""), subject, p[0]))
    ...
    page = _get({
        ...
        "pithumbsize": 1600, "inprop": "url", "cllimit": 30, "redirects": 1,
    })
    ...
    cats = [str(x.get("title", "")).replace("Thể loại:", "") for x in data.get("categories", [])]
...
def sentence_score(s, subject):
    n = _normalize(s)
    score = 8 if _normalize(subject) in n else 0
    if re.search(r"\b(18|19|20)\d{2}\b", s):
        score += 5
    if any(w in n for w in ("sinh", "thành lập", "ra mắt", "chiến dịch", "phát triển", "được bổ nhiệm", "sáng lập", "thành công", "qua đời", "phát minh", "xây dựng")):
        score += 4
    if any(f in n for f in BANNED_FILLER):
        score -= 20
    return score
...
def _historical_scope(topic):
    n = _normalize(topic)
    return any(p in n for p in ("lich su", "cuoc doi", "tieu su", "su nghiep", "hanh trinh"))
...
def _balanced_facts(sentences, subject, topic, limit=18):
    ...
    dated.sort(key=lambda x: (x[2] or 9999, x[0]))
    chosen = []
    if dated:
...
def _entities(s, canonical):
    entities = [canonical]
    for name in re.findall(r"\b[A-ZÀ-ỸĐ][\wÀ-ỹĐđ.-]+(?:\s+[A-ZÀ-ỸĐ][\wÀ-ỹĐđ.-]+){0,4}", s):
...
def _places(s):
    return list(dict.fromkeys(re.findall(r"(?:tại|ở|đến|từ)\s+([A-ZÀ-ỸĐ][\wÀ-ỹĐđ.-]+(?:\s+[A-ZÀ-ỸĐ][\wÀ-ỹĐđ.-]+){0,3})", s)))[:4]


def _scene_visual_plan(s, canonical, index):
    year = str(_year(s) or "")
    places = _places(s)
    entities = _entities(s, canonical)
    event = re.sub(r"\s+", " ", s).strip()[:150]
    queries = [canonical, f'"{canonical}" {year}'.strip(), f'"{canonical}" {event[:70]}'.strip()]
    if places:
        queries.append(f'"{canonical}" {places[0]}')
    queries.append(f'{event[:90]} historical photo')
    return {
        "mainCharacter": canonical, "event": event, "time": year,
        "location": places[0] if places else "", "entities": entities,
        "assetQueries": list(dict.fromkeys(q for q in queries if q.strip())),
        "icons": ["timeline", "document"] if year else ["document", "map"],
        "dataLayers": [year] if year else [], "secondaryObjects": entities[1:3],
        "paperElements": ["newspaper", "grid"], "background": "paper-grid",
        "camera": ["push-in", "pan-left", "pan-right", "parallax"][index % 4],
        "transition": "paper-swipe", "highlight": year or canonical,
    }
...
def build_story(story, topic):
    ...
    for i, scene in enumerate(scenes):
        fact = facts[min(i, len(facts) - 1)]
        scene["narration"] = fact
        scene["headline"] = subject if i == 0 else (str(_year(fact) or "") or subject)
        scene["keywords"] = list(dict.fromkeys([subject, *_places(fact), str(_year(fact) or "")]))[:5]
        scene["visualPlan"] = _scene_visual_plan(fact, subject, i)
    return story
