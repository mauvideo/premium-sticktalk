...
def keyword(scene):
 ...
 return {'primary':qs[0] if qs else p.get('mainSubject','documentary subject'),'secondary':qs[1:],'asset_type':'photo','style':'editorial documentary paper collage','emotion':'documentary','subject':p.get('mainSubject','documentary subject'),'subject_type':p.get('mainSubjectType','concept'),'identity_required':False,'event':p.get('event',''),'setting':p.get('location',''),'time_period':p.get('timePeriod',''),'visual_evidence':p.get('visualEvidence') or [],'template':VOX_TEMPLATE,'size':'9:16'}
...
class AssetManager:
 ...
 def providers(self):
  # Không dùng Wikimedia/Wikipedia image. Ưu tiên kho ảnh phổ biến, rồi Openverse.
 ...
 def variants(self,q,contextual=False):
  ...
  if contextual:
   ...
   # visualQueries của Gemini/scene luôn đứng trước chủ thể chung.
   out += [q.get('primary'),*(q.get('secondary') or []),s]
   if q.get('subject_type')=='person':out += [f'{s} portrait',f'{s} speaking',f'{s} event']
   elif s:out += [f'{s} documentary photo',f'{s} real photo']
 ...
 def generate(self,story):
  ...
  manifest=[];gallery=[];max_gallery=max(12,len(scenes)*3);print(f'IMAGE ENGINE subject={canonical!r} type={entity_type} sources=Pexels,Pixabay,Unsplash,Openverse')
  for idx,scene in enumerate(scenes,1):
   ...
   if not main:
    # Không lấy đại ảnh sai chủ đề: thử lại bằng chính headline/event của scene.
    fallback={**q,'primary':self._clean(scene.get('headline') or scene.get('event') or canonical),'secondary':[]}
    main=self.search(fallback,idx,contextual=False)
   if not main:raise RuntimeError(f'Không tìm được ảnh web phù hợp cho scene {idx} / chủ thể {canonical!r}; dừng thay vì render ảnh sai chủ đề.')
   ...
   scene['assets']=[]
   # 90s có nhiều cảnh: mỗi cảnh tự lấy tối đa 2 ảnh phụ riêng, không tái dùng ảnh cũ.
   attempts=0
   while len(scene['assets'])<2 and attempts<7:
  ...
  story['assetProviderSystem']='vox-scene-grounded-stock-assets-v10';story['template']=VOX_TEMPLATE;story['topicImageGallery']=gallery[:max_gallery];story['topicSubjectImages']=len(gallery);story['topicImageMinimumMet']=len(gallery)>=MIN_TOPIC_PHOTOS;story['resolvedEntityType']=entity_type
