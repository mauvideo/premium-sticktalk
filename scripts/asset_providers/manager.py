...
from .base import is_valid_mime,file_decodable
...
class AssetManager:
 ...
 def context_variants(self,scene,q,canonical):
  ...
  return out[:14]
 def valid(self,r):
  return bool(r and Path(r.file).exists() and Path(r.file).stat().st_size>700 and is_valid_mime(r.mime_type) and r.provider in ONLINE_PROVIDERS and file_decodable(Path(r.file),r.mime_type))
 ...
 def search_best(self,q,scene_index,texts,identity=False,canonical='',strict=True):
  ...
  for qi,text in enumerate(texts[:12]):
   ...
   for provider in self.providers(identity):
    try:
     ...
     if self.valid(r):
      s=self.score(r,qi,identity);candidates.append((s,r,text,provider.name));print(f'ASSET CANDIDATE scene {scene_index} {provider.name} score={s:.1f} query={text!r}')
     elif r:
      print(f'ASSET REJECT CORRUPT scene {scene_index} {provider.name}: {getattr(r,"file","")}')
    except Exception as e:print(f'ASSET SKIP {provider.name} {text!r}: {e}')
  if not candidates:return None
  candidates.sort(key=lambda x:x[0],reverse=True);threshold=55 if identity else 28
  ...
  if not strict:
   ...
   if s>=10:self.used.update({r.source_url,r.asset_id});print(f'ASSET SOFT PICK scene {scene_index}: {provider_name} score={s:.1f} query={text!r}');return r
  print(f'ASSET NO PICK scene {scene_index}: best={candidates[0][0]:.1f} threshold={threshold}');return None
 ...
 def generate(self,story):
  ...
  print(f'IMAGE ENGINE subject={canonical!r} type={entity_type} identity={identity} mode=multi-source-ranking-v18-decode-safe')
  for idx,scene in enumerate(scenes,1):
   ...
   if not main and identity and last_exact:print(f'ASSET IDENTITY REUSE scene {idx}');main=last_exact;fallback=True;role='named-entity-reuse'
   if not main and last_topic:print(f'ASSET TOPIC REUSE scene {idx}');main=last_topic;fallback=True;role='topic-reuse'
   if not main:raise RuntimeError(f'Không tìm được bất kỳ ảnh hợp lệ nào từ 4 nguồn cho phân cảnh {idx}, chủ đề {canonical!r}.')
   if not self.valid(main):raise RuntimeError(f'Ảnh cảnh {idx} không vượt qua kiểm tra decode trước render: {main.file}')
   if identity and role.startswith('named-entity'):last_exact=main
   last_topic=main;main_path=Path(main.file).as_posix();scene['image']=main_path;scene['asset']=main_path;scene['assetProvider']=main.provider;scene['assetSource']=main.source_url;scene['topicMatched']=True;scene['identityAnchored']=identity and role.startswith('named-entity');scene['semanticImageFallback']=fallback;scene['assetRole']=role
   ...
   for _ in range(2):
    ...
    if p==main_path or p in scene['assets'] or not self.valid(extra):continue
  ...
  # Cổng cuối: không ghi story cho Remotion nếu còn bất kỳ file ảnh hỏng nào.
  for idx,scene in enumerate(scenes,1):
   for p in [scene.get('image'),*(scene.get('assets') or [])]:
    if p and not file_decodable(Path(p)):
     raise RuntimeError(f'PRE-RENDER IMAGE CHECK FAILED scene {idx}: {p}')
  story['assetProviderSystem']='vox-multi-source-ranking-v18-decode-safe';story['template']=VOX_TEMPLATE;story['topicImageGallery']=gallery[:max_gallery];story['topicSubjectImages']=len(gallery);story['topicImageMinimumMet']=len(gallery)>=MIN_TOPIC_PHOTOS;story['resolvedEntityType']=entity_type
  self.story_path.write_text(json.dumps(story,ensure_ascii=False,indent=2),encoding='utf-8');(self.output_dir/'asset-manifest.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8');(self.output_dir/'credits.txt').write_text('\n'.join(['Asset credits','=============','']+[f"Scene {x['scene']}: {x['provider']} — {x['source_url']} — role={x['role']}" for x in manifest])+'\n',encoding='utf-8');print(f'IMAGE ENGINE OK: {len(gallery)} unique decode-safe images');return story
