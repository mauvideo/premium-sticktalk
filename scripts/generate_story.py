...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--idea','--y-tuong','--ý-tưởng',required=True); p.add_argument('--duration','--thoi-luong','--thời-lượng',type=int,default=45); p.add_argument('--tone','--giong-dieu','--giọng-điệu',default='sâu sắc'); p.add_argument('--format','--content-format','--dinh-dang','--định-dạng',dest='content_format',default='hybrid'); p.add_argument('--style','--phong-cach','--phong-cách',default='dark_neon'); p.add_argument('--voice','--giong-doc','--giọng-đọc',default='nam_bac_news'); p.add_argument('--motion-level','--muc-do-chuyen-dong','--mức-độ-chuyển-động',default='medium'); a=p.parse_args()
