...
const SupportingPhotos:React.FC<{items:string[];index:number;travel:number}>=({items,index,travel})=>{
  ...
  return <div style={{position:'absolute',top:405,right:48,width:270,display:'grid',gap:22,zIndex:7,transform:`translateX(${travel*.08}px)`}}>
    {photos.map((src,i)=><div key={`${src}-${i}`} style={{height:180,background:'#fffdf5',padding:9,clipPath:tear(index+i+40),boxShadow:`9px 10px 0 ${i?'#d62b22':'#ffd400'}`,transform:`rotate(${i?2.5:-2.5}deg)`}}><Img src={assetUrl(src)} style={{width:'100%',height:'100%',objectFit:'cover',filter:'grayscale(1) contrast(1.08)',clipPath:tear(index+i+43)}}/></div>)}
  </div>;
};
...
export const VoxScene:React.FC<VoxSceneProps>=({scene,story,sceneIndex})=>{
  ...
  // Fixed editorial zones: evidence occupies the upper-right, while icons live
  // in a dedicated middle/lower strip. Never place both icon slots in the same
  // region and keep them above the subtitle safe area.
  const iconSlots=[
    {left:70,top:1120,rotate:-6},
    {left:770,top:1115,rotate:6},
  ];
  ...
  return <AbsoluteFill style={{overflow:'hidden',background:'#f4efdf'}}>
    <div style={{position:'absolute',inset:-45,transform:`translateX(${tx}px) scale(${scale}) rotate(${rot}deg)`}}>
      ...
      <div style={{position:'absolute',left:40,right:40,top:720,height:330,zIndex:8,pointerEvents:'none'}}>
